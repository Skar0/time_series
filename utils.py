...
def split_sequence_nn_with_past_multi_step(sequence, n_steps_in, n_steps_out):
    ...
    for i in range(365 - n_steps_in, len(sequence)):
        ...
        # gather input and output parts of the pattern
        seq_x, seq_y = np.append(sequence[end_ix - 365:out_end_ix - 365], sequence[i:end_ix]), sequence[end_ix:out_end_ix]
        X.append(seq_x)
        y.append(seq_y)
    return np.array(X), np.array(y)
...
def split_sequence_nn_with_past_outliers_multi_step(sequence, sequence_with_outliers, n_steps_in, n_steps_out):
    ...
    for i in range(365 - n_steps_in, len(sequence)):
        ...
        # gather input and output parts of the pattern
        seq_x, seq_y = np.append(np.array(sequence_with_outliers[end_ix - 365:out_end_ix - 365]), np.array(sequence[i:end_ix])), sequence[end_ix:out_end_ix]
        X.append(seq_x)
        y.append(seq_y)
    return np.array(X), np.array(y)